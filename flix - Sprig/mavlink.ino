...
void sendMavlink() {
	...
	if (telemetrySlow) {
		...
		// Prefer the fuel-gauge's native state-of-charge if available (set by
		// power.ino from the MAX17048). Otherwise fall back to a linear
		// voltage-to-remaining approximation for the analog-only path.
		float remaining = isfinite(batteryRemaining)
			? batteryRemaining
			: constrain(mapf(voltage, 3.4, 4.2, 0, 1), 0, 1);
